...
(function (root, factory) {
  ...
  const AI_GENERATOR_RE = /openai|chatgpt|dall|sora|firefly|generative\s?(?:fill|expand|ai)|midjourney|stability|stable[\s_-]?diffusion|\bimagen\b|\bgemini\b|google\s?(?:ai|deepmind|labs)|microsoft\s?designer|bing\s?image|\bmeta\s?ai\b|imagine\.meta|ideogram|leonardo|recraft|runway|\bpika\b|\bkling\b|\bluma\b|\bveo\b|\bgrok\b|\bxai\b|black\s?forest|\bflux\b|\bkrea\b|canva.*(?:magic|ai)|getimg|playground|comfyui|automatic1111|novelai|invokeai|fooocus|synthesia|heygen|\bd-id\b|adobe\s?(?:express|photoshop).*generat/i;
  ...
  function findDisclosures(text, opts = {}) {
    ...
    for (const { level, re } of DISCLOSURE_PATTERNS) {
      ...
      while ((m = g.exec(text)) && guard++ < 200) {
        ...
        out.push({ level, match: m[0], context, scope: inferScope(context), index: m.index });
        if (out.length >= max) return dedupe(out);
      }
    }
    return dedupe(out);
  }
