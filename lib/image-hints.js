...
(function (root, factory) {
  ...
  function analyzeImageHints(h) {
    ...
    let hostname = '';
    let pathname = '';
    /* A trailing dot names the same host, so it is dropped before the
     * generator-host patterns below are matched against it. */
    try { const u = new URL(url); hostname = u.hostname.replace(/\.+$/, ''); pathname = u.pathname; } catch (e) { /* data: or relative */ }
