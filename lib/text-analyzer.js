...
(function (root, factory) {
  ...
  function analyzeText(text, opts = {}) {
    ...
    const lang = (opts.lang || '').toLowerCase().slice(0, 2);
    ...
    if (!clean) return finish({ signals, words, score: 0, verdict: 'no-signal', disclosures: [] });

    /* 1. Disclosures. */
    const disclosures = S.findDisclosures(clean);
    ...
    const selfStrong = raw.match(S.SELF_REFERENCE_STRONG_RE);
    if (selfStrong) {
      ...
      const selfMed = raw.match(S.SELF_REFERENCE_MEDIUM_RE);
      if (selfMed && words > 20) {
    ...
    const stylometry = words >= minWords ? scanStylometry(clean, words, mode) : null;
    if (stylometry) {
    ...
    return finish({ signals, words, score, verdict, disclosures, hidden: hidden.summary, stylometry: stylometry && stylometry.stats });
  }
