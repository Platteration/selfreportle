...
(function (root, factory) {
  ...
  function analyzeSite(snap) {
    ...
    const hostname = (snap.hostname || '').toLowerCase();
    const metas = snap.metas || [];
    const generator = metas.filter((m) => /^generator$/i.test(m.name || '')).map((m) => m.content || '').join(' | ');
    ...
    for (const fp of S.SITE_FINGERPRINTS) {
      const evidence = [];
      if (fp.meta && generator && fp.meta.some((re) => re.test(generator))) evidence.push('meta generator "' + generator.slice(0, 80) + '"');
    ...
    for (const m of metas) {
      ...
      if (S.META_DISCLOSURE_FALSE_RE.test(val)) {
        signals.push({ id: 'meta-human', kind: 'disclosure', weight: 0, label: 'Meta tag declares no AI involvement', detail: '<meta ' + (m.name ? 'name' : 'property') + '="' + key + '" content="' + val.slice(0, 60) + '">' });
      } else {
        disclosed = true;
        score = Math.max(score, 0.9);
        signals.push({ id: 'meta-ai', kind: 'disclosure', weight: 0.9, label: 'Meta tag declares AI-generated content', detail: '<meta ' + (m.name ? 'name' : 'property') + '="' + key + '" content="' + val.slice(0, 60) + '">' });
      }
    }
    ...
    for (const raw of snap.jsonLd || []) {
      ...
      if (creator && S.AI_GENERATOR_RE.test(creator[1]) && !/^(?:google|meta)$/i.test(creator[1].trim())) {
    ...
    const disclosures = S.findDisclosures(bodyText, { lang: snap.lang && snap.lang.slice(0, 2) });
