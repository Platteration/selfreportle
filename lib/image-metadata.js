...
(function (root, factory) {
  ...
  async function analyzeImageBytes(input, hints = {}) {
    ...
    /* Whether these bytes were shown to be the picture the page is rendering,
     * or are only what some fetch of the URL answered with. See
     * deriveSignals, and showsTheseBytes in content/content.js. */
    if (meta.c2pa && !meta.rendered) meta.notes.push('These credentials were checked against a separate fetch of this URL, and nothing shows the bytes carrying them are the picture on the page');
  ...
  /*
   * How many boxes one image's worth of JUMBF may cost.
   *
   * The parser walks a length-prefixed chain, so the number of boxes in a
   * region is chosen by whoever wrote the region: sixteen-byte boxes give a
   * quarter of a million of them per four megabytes, which is the fetch cap.
   * The fallback scan below then multiplies that by its 64 attempts, each
   * walking the whole remaining buffer, and a valid, decodable PNG with a
   * lattice of jumb/jumd boxes appended measured 54 s of the one service
   * worker every tab shares — four of them at a time, on an image that
   * passes every size and load gate. FETCH_TIMEOUT does not bound it: the
   * cost is in parsing bytes already in hand.
   *
   * The ceiling therefore sits in the parser rather than in any one caller,
   * so chunk, segment, box and scan all get it, and the scan shares one
   * allowance across all of its attempts. A conformant manifest store is a
   * few dozen boxes; a store that needs more than this is not one, and
   * running out means the boxes do not fill their container, which is
   * already what refuses the store an exclusion range (see storeFills).
   */
  const MAX_JUMBF_BOXES = 4096;

  function parseJumbfBoxes(b, start, end, depth = 0, allowance) {
    const boxes = [];
    const left = allowance || { boxes: MAX_JUMBF_BOXES };
    let p = start;
    while (p + 8 <= end && depth < 12) {
      if (left.boxes <= 0) break;
      left.boxes--;
      let len = u32be(b, p);
      const type = ascii(b, p + 4, 4);
      let hdr = 8;
      ...
      if (type === 'jumb') {
        const children = parseJumbfBoxes(b, bodyStart, bodyEnd, depth + 1, left);
  ...
  /* One allowance for the whole scan, not one per attempt: 64 attempts each
   * walking the rest of the file is the shape that cost 54 s. */
  function scanForJumbf(b) {
    const allowance = { boxes: MAX_JUMBF_BOXES };
    let from = 0;
    for (let i = 0; i < 64 && allowance.boxes > 0; i++) {
      const idx = indexOf(b, 'jumb', from);
      if (idx < 4) return null;
      if (ascii(b, idx + 8, 4) === 'jumd') {
        const boxes = parseJumbfBoxes(b, idx - 4, b.length, 0, allowance);
  ...
  function deriveSignals(meta) {
    ...
    if (meta.c2pa && meta.c2pa.active) {
      ...
      if (aiCreated) push({ id: 'c2pa-ai-created', hard: true, verdict: 'ai-generated', strength: 0.98, label: 'Content Credentials: created by generative AI', detail: base + ' Action ' + (created[0] ? created[0].action : 'assertion') + ' = ' + aiCreated.key + (agentAI ? ' via ' + agentAI : '') + '.' + signer });
         ...
         *   rendered — these bytes were decoded in the page and matched,
         *              pixel for pixel, against the picture the element is
         *              showing. The worker's own fetch is a second,
         *              distinguishable request (no cookies, a Range header,
         *              no Referer), and the page's own HTTP cache can be
         *              overwritten after an <img> has decoded, so without
         *              that match a server or a page can hand the reader an
         *              AI picture and the extension a signed photograph and
         *              have the badge land on the one nobody hashed.
         *
        ...
        const why = !vs || !vs.ok ? (vs && vs.text ? vs.text + '.' : 'The manifest was not verified.')
          : !vs.anchored ? 'The signature verified, but no certificate on its chain is a signer this build knows, so the name on it vouches for nothing.'
            : 'These bytes were not shown to be the picture on the page: they came from a separate fetch, or from a cache entry that does not decode to what the element is showing.';
