...
(function (root, factory) {
  ...
  async function analyzeImageBytes(input, hints = {}) {
    ...
    const meta = { format: detectFormat(bytes), exif: null, xmp: null, png: null, c2pa: null, c2paRanges: [], comments: [], notes: [] };
    try {
      ...
      if (!meta.c2pa) {
        ...
        if (j) { const c = extractC2pa(j); if (c) { meta.c2pa = c; meta.c2paRanges = [{ start: j[0].start, end: j[0].end }]; } }
      }
    } catch (e) {
  ...
  async function parseJpeg(b, meta) {
    ...
    while (p + 4 <= b.length) {
      ...
      if (marker === 0xe1) {
        ...
        entry.push({ z, body: seg.subarray(8), start: p, end: p + 2 + len });
        jumbf.set(en, entry);
      } else if (marker === 0xfe) {
    ...
    for (const parts of jumbf.values()) {
      ...
      if (c2pa) {
        meta.c2pa = c2pa;
        meta.c2paRanges = parts.map((part) => ({ start: part.start, end: part.end }));
        break;
      }
    }
  }
  ...
  async function parsePng(b, meta) {
    ...
    while (p + 8 <= b.length) {
      ...
      if (type === 'tEXt') {
        ...
        if (c2pa) { meta.c2pa = c2pa; meta.c2paRanges = [{ start: p, end: Math.min(p + 12 + len, b.length) }]; }
      }
      p += 12 + len;
    }
    meta.png = { text };
  ...
  async function parseWebp(b, meta) {
    ...
    while (p + 8 <= b.length) {
      ...
      if (type === 'EXIF') meta.exif = parseTiff(startsWith(data, 'Exif\0\0') ? data.subarray(6) : data) || meta.exif;
        ...
        if (c) { meta.c2pa = c; meta.c2paRanges = [{ start: p, end: Math.min(p + 8 + len + (len & 1), b.length) }]; }
      }
      p += 8 + len + (len & 1);
    }
  }
  ...
  async function parseIsobmff(bytes, meta) {
    ...
    if (!meta.c2pa) {
      ...
      if (scanned) {
        ...
        if (c) { meta.c2pa = c; meta.c2paRanges = [{ start: scanned[0].start, end: scanned[0].end }]; }
      }
    }
  }
  ...
  function walkIso(b, start, end, meta, depth) {
    ...
    while (p + 8 <= end) {
      ...
      if (type === 'uuid' && hex(b, bodyStart, 16) === C2PA_UUID) {
        ...
        const c = extractC2pa(parseJumbfBoxes(data, 0, data.length));
        if (c && !meta.c2pa) { meta.c2pa = c; meta.c2paRanges = [{ start: p, end: bodyEnd }]; }
  ...
  function deriveSignals(meta) {
    ...
    if (meta.c2pa && meta.c2pa.active) {
      ...
      if (aiCreated) push({ id: 'c2pa-ai-created', hard: true, verdict: 'ai-generated', strength: 0.98, label: 'Content Credentials: created by generative AI', detail: base + ' Action ' + (created[0] ? created[0].action : 'assertion') + ' = ' + aiCreated.key + (agentAI ? ' via ' + agentAI : '') + '.' + signer });
         ...
         * and forging it costs nothing when nobody checks: a hand-written
         * JUMBF box with no certificate and no signature would otherwise earn
         * the same green badge as a signed and bound manifest. So those
         * claims only count when the manifest actually verified; otherwise
         * they are reported as what they are, an unverified assertion.
         */
        const proven = !!(vs && vs.ok);
        const unproven = (id, d, why) => push({
          id, hard: false, verdict: 'no-signal', strength: 0,
          label: 'Content Credentials claim ' + d + ', unverified',
          detail: base + signer + ' ' + why + ' Nothing here shows the claim belongs to this file, so it is not read as provenance.',
        });
        if (capture) {
          ...
          else unproven('c2pa-capture-unverified', capture.label.toLowerCase(), vs && vs.text ? vs.text + '.' : 'The manifest was not verified.');
        } else if (human) {
          ...
          else unproven('c2pa-human-unverified', human.label.toLowerCase(), vs && vs.text ? vs.text + '.' : 'The manifest was not verified.');
        } else if (algo) {
          ...
          else unproven('c2pa-algo-unverified', algo.label.toLowerCase(), vs && vs.text ? vs.text + '.' : 'The manifest was not verified.');
          ...
          else unproven('c2pa-capture-device-unverified', 'a capture device or app', vs && vs.text ? vs.text + '.' : 'The manifest was not verified.');
        } else {
