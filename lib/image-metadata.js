...
(function (root, factory) {
  ...
  async function analyzeImageBytes(input, hints = {}) {
    ...
    if (meta.c2pa && meta.c2pa.active && VERIFY) {
      try {
        ...
        meta.c2pa.active.verification = { signature: 'unknown', notes: ['Verification failed: ' + (e && e.message)] };
      }
    }
  ...
  async function parseIsobmff(bytes, meta) {
    ...
    if (!meta.c2pa) {
      ...
      if (scanned) meta.c2pa = extractC2pa(scanned);
    }
  }
  ...
  function parseManifest(box) {
    ...
    for (const a of (assertionsBox ? assertionsBox.children : []).filter((c) => c.type === 'jumb')) {
      ...
      m.assertionBoxes.push({ label, raw: a.raw, content: inner ? inner.raw : null });
  ...
  function deriveSignals(meta) {
    ...
    if (meta.c2pa && meta.c2pa.active) {
      ...
      if (vs && vs.broken) {
        push({ id: 'c2pa-broken', hard: true, verdict: 'suspected', strength: 0.6, label: 'Content Credentials do not verify', detail: base + signer + ' ' + vs.text + '. Treat the claims inside this manifest as unreliable.' });
        ...
        push({ id: 'c2pa-verified', hard: false, verdict: 'no-signal', strength: 0, label: 'Content Credentials signature verified', detail: vs.text + '. A valid signature shows the manifest is intact since signing; it does not by itself prove who signed it.' });
      } else if (v) {
  ...
  function summarize(meta) {
    ...
    if (meta.c2pa) {
      ...
      out.c2pa = {
        ...
        ingredients: a.ingredients, signerNames: a.signerNames, digitalSourceTypes: a.digitalSourceTypes,
        softwareAgents: a.softwareAgents,
