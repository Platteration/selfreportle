...
(function (root, factory) {
  ...
  async function analyzeImageBytes(input, hints = {}) {
    ...
    if (meta.c2pa && meta.c2pa.active && VERIFY) {
      try {
        meta.c2pa.active.verification = await VERIFY.verifyManifest(meta.c2pa.active, { truncated: !!hints.truncated });
      } catch (e) {
  ...
  function parseManifest(box) {
    ...
    const refs = m.claim
      ...
      : [];
    // A claim that references nothing gives no basis to restrict by, so the
    // assertions are read as an unverified claim and reported as one.
    m.referencedAssertions = refs.length ? refs : null;
    ...
    for (const a of (assertionsBox ? assertionsBox.children : []).filter((c) => c.type === 'jumb')) {
      ...
      m.assertionBoxes.push({ label, raw: a.raw, content: inner ? inner.data : null });
  ...
  function deriveSignals(meta) {
    ...
    if (meta.c2pa && meta.c2pa.active) {
      ...
      if (vs && vs.broken) {
        push({ id: 'c2pa-broken', hard: true, verdict: 'suspected', strength: 0.7, label: vs.payloadMismatch ? 'Content Credentials carry a signature over different content' : 'Content Credentials do not verify', detail: base + signer + ' ' + vs.text + '. Treat every claim inside this manifest as unreliable.' });
  ...
  /* null means "no basis to restrict", which is the pre-verification case. */
