 ...
 *
 * Limitations (stated in the UI): C2PA signatures are parsed, not
 * cryptographically verified; invisible pixel-domain watermarks (SynthID,
 * Stable Signature …) need vendor keys and are not detected.
 */
(function (root, factory) {
  ...
  async function analyzeImageBytes(input, hints = {}) {
    ...
    const meta = { format: detectFormat(bytes), exif: null, xmp: null, png: null, c2pa: null, comments: [], notes: [] };
    try {
      ...
      else if (meta.format === 'isobmff') await parseIsobmff(bytes, meta);
      ...
      if (!meta.c2pa) { const j = scanForJumbf(bytes); if (j) meta.c2pa = extractC2pa(j); }
    } catch (e) {
  ...
  async function parseJpeg(b, meta) {
    ...
    while (p + 4 <= b.length) {
      ...
      if (marker === 0xe1) {
        ...
        entry.push({ z, body: seg.subarray(8) });
        jumbf.set(en, entry);
      } else if (marker === 0xfe) {
  ...
  async function parsePng(b, meta) {
    ...
    while (p + 8 <= b.length) {
      ...
      if (type === 'tEXt') {
        ...
        if (z > 0) text[latin1.decode(data.subarray(0, z))] = latin1.decode(data.subarray(z + 1));
      } else if (type === 'zTXt') {
        const z = data.indexOf(0);
        if (z > 0) {
          const inflated = await inflate(data.subarray(z + 2));
          if (inflated) text[latin1.decode(data.subarray(0, z))] = latin1.decode(inflated);
        }
      } else if (type === 'iTXt') {
        const z = data.indexOf(0);
        if (z > 0) {
          ...
          const raw = compressed ? await inflate(body) : body;
          if (raw) text[key] = utf8.decode(raw);
        }
      } else if (type === 'eXIf') {
  ...
  async function parseWebp(b, meta) {
    ...
    while (p + 8 <= b.length) {
      ...
      else if (type === 'C2PA') { const c = extractC2pa(parseJumbfBoxes(data, 0, data.length)); if (c) meta.c2pa = c; }
      p += 8 + len + (len & 1);
    }
  }
  ...
  async function parseIsobmff(b, meta) {
    let p = 0;
    while (p + 8 <= b.length) {
      let len = u32be(b, p);
      const type = ascii(b, p + 4, 4);
      ...
      if (len < hdr) break;
      if (type === 'uuid' && hex(b, p + hdr, 16) === C2PA_UUID) {
        const data = b.subarray(p + hdr + 16 + 4, Math.min(p + len, b.length)); // skip version/flags (4)
        const c = extractC2pa(parseJumbfBoxes(data, 0, data.length));
        if (c) meta.c2pa = c;
      }
      p += len;
    }
    // EXIF / XMP items live inside meta/iloc; fall back to a byte scan.
    const exifIdx = indexOf(b, 'Exif\0\0');
    if (exifIdx >= 0) meta.exif = parseTiff(b.subarray(exifIdx + 6)) || meta.exif;
  }
  ...
  function parseSvg(b, meta) {
    ...
    for (const c of comments) meta.comments.push(c.slice(4, -3).trim().slice(0, 300));
    const gen = text.match(/<(?:dc:)?(?:creator|generator|title)[^>]*>([^<]{2,120})</i);
  ...
  function xmpValue(xml, local) {
    const attr = new RegExp('(?:^|[\\s<"])(?:[\\w-]+:)?' + local + '\\s*=\\s*"([^"]*)"', 'i').exec(xml);
    if (attr) return attr[1].trim();
    const el = new RegExp('<(?:[\\w-]+:)?' + local + '\\b([^>]*)>([\\s\\S]*?)</(?:[\\w-]+:)?' + local + '>', 'i').exec(xml);
    if (el) {
      const li = /<rdf:li[^>]*>([\s\S]*?)<\/rdf:li>/i.exec(el[2]);
      const inner = li ? li[1] : el[2];
      const res = /rdf:resource\s*=\s*"([^"]*)"/i.exec(inner) || /rdf:resource\s*=\s*"([^"]*)"/i.exec(el[1]);
      const textVal = inner.replace(/<[^>]+>/g, '').trim();
      return textVal || (res ? res[1].trim() : '');
    }
    const self = new RegExp('<(?:[\\w-]+:)?' + local + '\\b[^>]*rdf:resource\\s*=\\s*"([^"]*)"[^>]*/>', 'i').exec(xml);
  ...
  function parseXmp(xml) {
    ...
    const x = {
      ...
      aiFlags: unique([...xml.matchAll(/<[\w-]+:(?:AIGenerated|ai[-_]?generated|GenAI\w*|AiGenerated)\b[^>]*>([^<]*)</gi)].map((m) => m[0].slice(0, 80))),
      length: xml.length,
    };
    return x;
  }
  ...
  function parseJumbfBoxes(b, start, end, depth = 0) {
    ...
    while (p + 8 <= end && depth < 12) {
      ...
      const box = { type, start: p, end: bodyEnd, data: b.subarray(bodyStart, bodyEnd), children: [], label: null, uuid: null };
      if (type === 'jumb') {
  ...
  function parseManifest(box) {
    ...
    for (const a of (assertionsBox ? assertionsBox.children : []).filter((c) => c.type === 'jumb')) {
      ...
      if (/^c2pa\.actions/.test(label)) {
        for (const act of Array.isArray(content.actions) ? content.actions : []) {
          ...
          const entry = { action: act.action || null, digitalSourceType: act.digitalSourceType || null, softwareAgent: agent, when: act.when || null, description: act.description || null };
          m.actions.push(entry);
          if (entry.digitalSourceType) m.digitalSourceTypes.push(entry.digitalSourceType);
          if (agent) m.softwareAgents.push(agent);
        }
        for (const t of Array.isArray(content.templates) ? content.templates : []) {
          if (t && t.digitalSourceType) m.digitalSourceTypes.push(t.digitalSourceType);
        ...
        if (dst) m.digitalSourceTypes.push(S.IPTC_DST_PREFIX + dst[1]);
    ...
    m.digitalSourceTypes = unique(m.digitalSourceTypes);
  ...
  function deriveSignals(meta) {
    ...
    if (meta.c2pa && meta.c2pa.active) {
      ...
      const created = a.actions.filter((x) => x.action === 'c2pa.created' || x.action === 'c2pa.placed');
      const dstAll = a.digitalSourceTypes.map(S.digitalSourceType).filter(Boolean);
      ...
      const agentAI = a.softwareAgents.find((x) => S.AI_GENERATOR_RE.test(x));
      const genAI = gen && S.AI_GENERATOR_RE.test(gen);
      const signer = a.signerNames.length ? ' Signed by: ' + a.signerNames.join(', ') + ' (signature not verified).' : ' (signature not verified)';
      ...
      if (aiCreated) push({ id: 'c2pa-ai-created', hard: true, verdict: 'ai-generated', strength: 0.98, label: 'Content Credentials: created by generative AI', detail: base + ' Action ' + (created[0] ? created[0].action : 'assertion') + ' = ' + aiCreated.key + (agentAI ? ' via ' + agentAI : '') + '.' + signer });
        ...
        if (capture) push({ id: 'c2pa-capture', hard: true, verdict: 'captured', strength: capture.strength, label: 'Content Credentials: ' + capture.label, detail: base + signer });
        else if (human) push({ id: 'c2pa-human', hard: true, verdict: 'human-created', strength: human.strength, label: 'Content Credentials: ' + human.label, detail: base + signer });
        else if (algo) push({ id: 'c2pa-algo', hard: true, verdict: 'algorithmic', strength: algo.strength, label: 'Content Credentials: ' + algo.label, detail: base + signer });
        else if (gen && S.CAPTURE_GENERATOR_RE.test(gen)) push({ id: 'c2pa-capture-device', hard: true, verdict: 'captured', strength: 0.75, label: 'Content Credentials from a capture device / app', detail: base + signer });
        else push({ id: 'c2pa-present', hard: false, verdict: 'no-signal', strength: 0, label: 'Content Credentials present (no AI action declared)', detail: base + ' Assertions: ' + a.assertions.join(', ') + '.' + signer });
      }
    ...
    if (e) {
      ...
      if (e.make && S.CAMERA_MAKE_RE.test(e.make) && !signals.some((s) => s.verdict === 'ai-generated')) push({ id: 'exif-camera', hard: false, verdict: 'captured', strength: 0.45, label: 'Camera EXIF present', detail: [e.make, e.model, e.lensModel].filter(Boolean).join(' ') + (e.dateTimeOriginal ? ', ' + e.dateTimeOriginal : '') + ' (EXIF can be forged or inherited)' });
    }
  ...
  function summarize(meta) {
    ...
    if (meta.c2pa) out.c2pa = { manifestCount: meta.c2pa.manifestCount, ...meta.c2pa.active };
  ...
  return { analyzeImageBytes, detectFormat, parseTiff, parseXmp, parseJumbfBoxes, extractC2pa, deriveSignals, scanForXmp, scanForJumbf, _internal: { concat, u32be } };
});
