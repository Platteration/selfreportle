...
(function (root, factory) {
  ...
  function attributeImage(signals, metadata) {
    ...
    /* "parameters" is the name of a PNG text chunk, so it only counts as a
     * marker when it is a key. Matched against the joined blob it also hit
     * ordinary prose: "Flight test parameters recorded at Cape Town." */
    const pngKeys = Object.keys(png);
    const sdMarkers = /Steps:\s*\d+|class_type|KSampler|"uc"\s*:|invokeai|fooocus/i.test(embedded)
      || pngKeys.some((k) => /^parameters$/i.test(k));
  ...
  function attributeSite(siteResult, snapshot) {
    ...
    if (s.builder && s.builder.kind === 'ai-builder') {
      const p = matchProfile(s.builder.name, ['site']);
      // Naming a real company because nothing matched would be worse than
      // saying the tool is unprofiled.
      if (!p) return { id: null, name: s.builder.name, vendor: null, country: null, confidence: 'confirmed', evidence: 'Generator fingerprint: ' + s.builder.name, detail: '', marking: null, skews: GENERIC_SITE_SKEWS };
    ...
    /* A credit left in the code is evidence; a statement on the page is a
     * declaration. Which one it was is known from the signal id, so read that
     * rather than sniffing the rendered text for the word "comment". */
    const sources = (s.signals || [])
      .filter((x) => ['code-comment', 'visible-site-disclosure', 'meta-ai', 'jsonld-creator', 'attr-ai'].includes(x.id))
      .map((x) => ({ confidence: x.id === 'code-comment' ? 'confirmed' : 'declared', detail: x.detail || '' }));
    for (const src of sources) {
      const hit = matchProfile(src.detail, ['site', 'code', 'text']);
      if (hit) return make(hit, src.confidence, src.detail.slice(0, 160));
    }
    if (s.builder && s.builder.kind === 'ai-host') {
      const host = matchProfile(s.builder.name, ['site']);
      if (host) return make(host, 'inferred', 'Hosted on ' + s.builder.name);
    }
