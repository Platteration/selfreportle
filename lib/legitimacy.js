...
(function (root, factory) {
  ...
  /* findVat looks for its numbers in a window with separators stripped, so
   * each format is needed without its optional hyphens. Compiling those
   * inside the per-hit loop cost 29 fresh RegExp objects for every context
   * word on the page: a body of "vat " repeated spent ~400 ms of the page's
   * own main thread here, once per SPA navigation. Compile them once. */
  const VAT_PATTERNS = Object.freeze(Object.entries(VAT_FORMATS).map(
    ([country, re]) => Object.freeze({ country, re: new RegExp(re.source.replace(/-\?/g, '')) })));
  ...
  const MAX_VAT_CONTEXT_HITS = 400;
  ...
  function findVat(text) {
    ...
    const seen = new Set();
    let m;
    /* Bounded by context hits as well as by findings: six numbers stop the
     * scan, but a page with ten thousand context words and no number at all
     * would otherwise run every format against every one of them. */
    let hits = 0;
    while ((m = ctx.exec(text)) && out.length < 6 && hits < MAX_VAT_CONTEXT_HITS) {
      hits++;
      const window = text.slice(m.index, m.index + 60).replace(/[\s.\u2013\u2014-]/g, '').toUpperCase();
      for (const { country, re } of VAT_PATTERNS) {
  ...
  return { analyzeLegitimacy, VAT_FORMATS, VAT_PATTERNS, MAX_VAT_CONTEXT_HITS, REGISTRATION_PATTERNS, PRESSURE_PATTERNS, CHECK_LABELS, findVat, findRegistrations };
});
