...
(function (root, factory) {
  ...
  const EMAIL_RE = /\b[\w.+-]+@([\w-]+\.)+[a-z]{2,12}\b/i;
  const POSTAL_RE = /\b(?:\d{4,5}\s+[A-Z][\w'’-]+|[A-Z]{1,2}\d{1,2}[A-Z]?\s*\d[A-Z]{2}|\d{5}(?:-\d{4})?)\b/;
  /* German street names are compounds ("Musterstrasse"), so the common
   * suffixes must match without a leading word boundary. */
  const STREET_RE = /(?:stra[ßs]e|str\.|[a-zäöüß]{3,}(?:weg|platz|allee|gasse|ring))\b|\b(?:street|st\.|road|rd\.|avenue|ave\.|lane|drive|boulevard|blvd\.|rue|voie|calle|avenida|via|viale|piazza|straat|laan|plein|gata|gatan|vej|ulica|utca|ulice)\b/i;
